...
public class RecyclerAdapter_first_annonce_banner extends RecyclerView.Adapter<RecyclerAdapter_first_annonce_banner.MyViewHolder> {
    ...
    List<contact_general.media> contactslist;
    RecyclerView recyclerView;

    public RecyclerAdapter_first_annonce_banner(Context context, List<contact_general.media> contactslist, RecyclerView recyclerView){
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, final int position) {
        try {
            ...
            Glide.with(context).load("https://storejoud.com/"+
                    contactslist.get(position).getSlider_images().get(0)).error(R.drawable.logo).into(holder.image);
        }
        catch (Exception e){}
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Glide.with(context).load("https://storejoud.com/"+
                        contactslist.get(position).getSlider_images().get(0)).error(R.drawable.logo).into(img);
                settingsDialog.show();
            }
